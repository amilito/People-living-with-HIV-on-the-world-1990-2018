...
new_names <-
  my_data %>% names(.) %>% .[5:nc] %>% lapply(., substr, start = 1, stop = 4) %>% unlist(.)
...
my_data_preprocess <-
  my_data %>% missing_values("..", NA, .) %>% .[, 5:33] %>% lapply(., function(x)
    as.numeric(as.character(x))) %>% cbind(part, .)
...
dataformap <-
  my_data %>% missing_values("..", NA, .) %>% .[, 5:33] %>% lapply(., function(x)
    as.numeric(as.character(x))) %>% cbind(part, .) %>% joinCountryData2Map(., joinCode =
                                                                              "NAME", nameJoinColumn = "Country Name")
